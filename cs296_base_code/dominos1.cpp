...
namespace cs296
{
  ...
  dominos_t::dominos_t()
  {
		...
		//! revolute joint between fake body and polygon with 6 vertices
                ...
                shape.SetAsBox(4.675f, 0.25f);
                bd.type = b2_dynamicBody;
                bd.position.Set(3.2f, 18.0f);
                bd.angle=-1;
...
rjd.upperAngle = 0.25f * b2_pi;rjd.lowerAngle = -0.15f * b2_pi;rjd.enableLimit=true;
rjd.enableMotor=true;
                m_world->CreateJoint(&rjd);rjd.enableMotor=false;rjd.enableLimit=false;
		...
		//! revolute joint between rod from 6 sided polygon and rod from it to top circle
                rjd.Initialize(rod_bw_rodFrom6sidedPoly_topCircle, rod_from_topPoly, b2Vec2(6.0f, 15.0f));
                m_world->CreateJoint(&rjd);             
                //! square piston               
		...
		//! revolute joint between circle on bounder and rod connecting bearing to diamond
                ...
                shape1.m_radius = 0.5f;
                bd.type = b2_dynamicBody;
                bd.position.Set(22.5f, 15.0f);
                b2Body* circle_on_top = m_world->CreateBody(&bd);
                fixtureDe.shape = &shape1;//fixtureDef.filter.groupIndex = -4;
                circle_on_top->CreateFixture(&fixtureDe);
		...
		//! bounder 6
                shape.SetAsBox(6.5f, 0.5f);
                bd.type = b2_staticBody;
                bd.position.Set(22.5f, 14.0f);
...
rjd.enableMotor=false;
                rjd.Initialize(rod_bw_rodFrom6sidedPoly_topCircle, circle_on_top, b2Vec2(22.5f, 15.0f));
                m_world->CreateJoint(&rjd);
		//! revolute joint between circle on top and rod between two circles
                rjd.enableMotor=false;rjd.Initialize(rod_bw_two_circles, circle_on_top, b2Vec2(22.5f, 15.0f));
                //m_world->CreateJoint(&rjd);
...
jointDef.Initialize(circle_on_top, rod_bw_two_circles, circle_on_top->GetWorldCenter(), worldAxis);
//m_world->CreateJoint(&jointDef);
		//! revolute joint between large piston and rod connected to it
                //rjd.Initialize(largePiston, rodRectPiston, b2Vec2(32.5f, 10.0f));
                //m_world->CreateJoint(&rjd);
		//! revolute joint between large piston and rod connected to it
                rjd.Initialize(largePiston, rodRectPiston, largePiston->GetWorldCenter());
		m_world->CreateJoint(&rjd);
		//jointDef.Initialize(largePiston, rodRectPiston, largePiston->GetWorldCenter(), b2Vec2(10.0f, 0.0f));
                //m_world->CreateJoint(&jointDef);
		//! revolute joint bw rod connecting two circles and rod connecting this rod to large rectangular piston
		rjd.Initialize(rod_bw_two_circles, rodRectPiston, b2Vec2(23.3f, 10.3f));
                m_world->CreateJoint(&rjd);
